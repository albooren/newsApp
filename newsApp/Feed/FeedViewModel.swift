...
class FeedViewModel {
    ...
    func getArticle(searchKeyWord : String,onCompleted: @escaping () -> (), onFailed: @escaping () -> ()) {
        NetworkManager.shared.request(with: getApiUrl(keyword: searchKeyWord)) { [weak self] (data: Welcome) in
    ...
    func getApiUrl(keyword : String) -> String{
        return NetworkParameters.baseURL + NetworkParameters.urlType + keyword + NetworkParameters.apiKey
    }
